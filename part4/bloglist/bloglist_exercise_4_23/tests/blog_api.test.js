...
describe("Blog API tests", () => {
  let token; // Declare token variable to store valid token
  ...
  describe("POST /api/blogs", () => {
    ...
    test("a valid blog can be added with a valid token", async () => {
      ...
      await api
        .post("/api/blogs")
        .set("Authorization", `Bearer ${token}`) // Provide valid token in headers
        .send(newBlog)
        .expect(201)
        .expect("Content-Type", /application\/json/);
    });
    ...
    test("adding a blog fails with 401 Unauthorized if token is not provided", async () => {
      ...
      await api
        .post("/api/blogs")
        .send(newBlog) // Do not provide token in headers
        .expect(401);
      // });
    });
